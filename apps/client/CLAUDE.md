...
- **Dados moram em hooks por feature** — `src/<feature>/queries.ts` expõe `useXQuery`/`useXMutation`. A tela consome o hook e, se precisar, passa callbacks de UI no próprio `mutate(vars, { onSuccess, onError })` (ex.: `Alert`, `setError`, haptics). Efeitos colaterais de cache (invalidação) ficam **dentro** do hook. **Exceção:** quando o botão usa `useGuardedPress` (ver abaixo), a tela chama `mutateAsync` com `try/catch` — a trava precisa da promise da ação para saber quando soltar.
- **Todo botão que dispara ação ou navegação passa por uma trava** (`src/ui/press-guard.ts`): `useGuardedPress` para ações que terminam (login, mutation, confirmação + delete) e `useGuardedNavigation` para `router.push`/`router.back`. `disabled={mutation.isPending}` sozinho **não** basta — o flag só chega ao botão no render seguinte, e ação que abre diálogo antes da requisição (`confirmAsync`) nem fica pendente enquanto o diálogo está aberto. Use `isRunning` para o estado visual (`opacity: 0.5` + label de progresso).
