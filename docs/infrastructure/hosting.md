...
> projetos. O plano operacional completo (setup do host, DNS/TLS, CD, backups, hardening)
> está em [deployment.md](deployment.md). Esta página é o resumo de alto nível.
...
| CI/CD | **GitHub Actions** | build/test + deploy (ver [cicd.md](cicd.md) / [deployment.md](deployment.md)) |
...
- Cada projeto é um **stack Docker Compose** isolado; um **Caddy interno** roteia por
  subdomínio (ver topologia em [deployment.md](deployment.md)).
...
- **Backups** automáticos (pg_dump + cópia offsite) — ver [deployment.md](deployment.md) §10.
